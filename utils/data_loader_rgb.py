...
def luptonize(img, Q=10, stretch=0.5, minimum=0):
    """ converts img to rgb channels """
    img_rgb =  make_lupton_rgb(img[:,:,2], img[:,:,1], img[:,:,0],
                         Q=Q, stretch=stretch, minimum=minimum)
    return img_rgb

class ToRGB:
  def __call__(self, image):
    return luptonize(image)

class Resize:
  def __init__(self, outdim=224):
    self.outdim = outdim

  def __call__(self, image):
    return (skimage.transform.resize(image, output_shape=(self.outdim,self.outdim), order=3)).astype(np.float32)
